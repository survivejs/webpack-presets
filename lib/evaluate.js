...
function evaluate(_ref) {
  ...
  var rcConfiguration = _webpackMerge2.default.apply(null, [webpackrc].concat((0, _parse.parse)(apply(presets, paths), webpackrc.presets)));
  ...
  var evaluatedActions = apply(actions, paths);
  var evaluatedFormats = apply(formats, paths);
  var rootConfig = {
    resolve: {
      extensions: ['']
    }
...
function apply(configuration, paths) {
  if (Array.isArray(configuration)) {
    return _webpackMerge2.default.apply(null, configuration.map(function (conf) {
      return conf(paths);
    }));
  }
